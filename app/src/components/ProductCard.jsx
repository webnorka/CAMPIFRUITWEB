import { Plus, Check, Tag, Eye } from 'lucide-react';
        ...
        <>
            <div className="card-bento group overflow-hidden flex flex-col h-full bg-white">
                ...
                <div className="relative aspect-square overflow-hidden m-3 sm:m-5 rounded-[1.5rem] sm:rounded-[2rem] bg-organic">
                    ...
                    {/* Quick view / Product detail link */}
                    <Link
                        to={`/producto/${product.slug || product.id}`}
                        className="absolute inset-0 flex items-center justify-center bg-forest/0 group-hover:bg-forest/30 transition-all duration-300 cursor-pointer"
                        aria-label={`Ver detalle de ${product.name}`}
                    >
                        <div className="w-12 h-12 bg-white/90 backdrop-blur-sm rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-300 shadow-xl">
                ...
                <div className="p-5 sm:p-8 pt-0 flex-1 flex flex-col">
                    ...
                    <Link to={`/producto/${product.slug || product.id}`} className="block">
                        <h3 className="text-lg sm:text-2xl font-display font-black text-forest mb-2 sm:mb-3 leading-tight tracking-tight hover:text-forest/70 transition-colors">
                            {product.name}
                        </h3>
                    </Link>
