        ...
        <section className="relative min-h-[92vh] flex items-center overflow-hidden pt-20">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-12 lg:px-20 w-full">
                <div className="max-w-3xl text-center md:text-left">
                    {/* Badge */}
                    <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-xl px-5 py-2.5 rounded-full border border-white/20 shadow-2xl mb-10 animate-fade-in mx-auto md:mx-0">
                        <span className="w-2.5 h-2.5 bg-accent rounded-full animate-pulse shadow-[0_0_15px_rgba(163,230,53,0.5)]" />
                        <span className="text-[10px] font-black text-white uppercase tracking-[0.3em]">Cosecha Premium Local</span>
                    </div>

                    {/* Main heading - High impact, high contrast */}
                    <h1 className="text-5xl sm:text-7xl md:text-8xl font-display font-black text-white mb-8 leading-[1] tracking-tight animate-slide-up drop-shadow-2xl">
                        {config.heroTitle || 'La frescura del campo en tu mesa'}
                    </h1>

                    {/* Subtitle - Optimized for readability */}
                    <p className="text-lg sm:text-2xl text-white/80 mb-12 max-w-2xl font-medium leading-relaxed animate-fade-in drop-shadow-lg mx-auto md:mx-0" style={{ animationDelay: '0.1s' }}>
                        {config.heroSubtitle || 'Frutas tropicales seleccionadas con pasión para deleitar tus sentidos.'}
                    </p>

                    {/* CTA Buttons - Mobile Optimized */}
                    <div className="flex flex-col sm:flex-row items-center gap-5 animate-fade-in mx-auto md:mx-0" style={{ animationDelay: '0.2s' }}>
                        <Link
                            to="/catalogo"
                            className="w-full sm:w-auto btn-primary group h-16 px-10 text-sm uppercase tracking-[0.2em]"
                        >
                            Explorar Catálogo
                            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                        </Link>

                        <div className="flex items-center gap-4 px-6 py-4 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/10 shadow-xl">
                            <div className="flex -space-x-3">
                                {[1, 2, 3].map(i => (
                                    <div key={i} className="w-9 h-9 rounded-full border-2 border-forest bg-accent flex items-center justify-center text-[10px] font-black text-forest">
                                        {i === 3 ? '+80' : ''}
                                    </div>
                                ))}
                            </div>
                            <span className="text-[10px] font-black text-white uppercase tracking-widest whitespace-nowrap">+80 negocios confían</span>
                        </div>
                    </div>
                </div>
            </div>
